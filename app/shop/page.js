...
export default function ShopPage() {
  ...
  const filteredProducts = filterBrand === 'All' 
    ? MOCK_PRODUCTS 
    : MOCK_PRODUCTS.filter(p => p.brand === filterBrand);
    ...
    <div className="pt-20 min-h-screen bg-carbon">
      ...
      <div className="relative w-full h-[40vh] overflow-hidden mb-12">
        ...
        <div className="absolute inset-0 flex items-center justify-center">
          <div className="text-center">
            <h1 className="text-5xl md:text-7xl font-syne font-extrabold text-tactical tracking-tighter uppercase drop-shadow-2xl">Elite Hardware</h1>
      ...
      <div className="container mx-auto px-4 pb-24 flex flex-col lg:flex-row gap-8">
        ...
        <aside className="w-full lg:w-1/4 glass-panel p-6 h-fit sticky top-24">
          <h2 className="text-xl font-orbitron text-tactical mb-6 uppercase border-b border-cyber/30 pb-2">Filters</h2>
          <div className="mb-6">
            <h3 className="text-white font-inter font-bold mb-3">Brands</h3>
            <div className="flex flex-col gap-2">
              {['All', 'NVIDIA', 'Intel', 'Dell', 'Logitech G'].map(brand => (
                <label key={brand} className="flex items-center gap-3 cursor-pointer group">
                  <div className={`w-4 h-4 rounded-sm border ${filterBrand === brand ? 'bg-tactical border-tactical' : 'border-cyber group-hover:border-tactical transition-colors'}`}></div>
                  <span className={`text-sm font-inter ${filterBrand === brand ? 'text-white' : 'text-cyber group-hover:text-white transition-colors'}`} onClick={() => setFilterBrand(brand)}>
                    {brand}
                  </span>
                </label>
              ))}
          ...
          {filteredProducts.map(product => (
            <div key={product.id} className="glass-panel group flex flex-col justify-between overflow-hidden relative">
              <div className="relative w-full h-48 overflow-hidden bg-black">
                ...
                <div className="absolute top-2 right-2 bg-carbon/80 backdrop-blur-sm border border-cyber/30 px-3 py-1 rounded-sm text-xs font-orbitron text-tactical">
                  {product.brand}
                </div>
              </div>
              <div className="p-6 flex flex-col flex-grow">
                ...
                <div className="flex items-end justify-between mt-auto">
                  <span className="text-2xl font-syne font-extrabold text-tactical">${product.price}</span>
                  <button 
                    onClick={() => addToCart(product)}
                    className="bg-tactical text-carbon font-inter font-bold px-4 py-2 text-sm uppercase rounded-sm hover:shadow-glow-tactical transition-all duration-300 transform hover:-translate-y-1"
                  >
                    Add to Cart
                  </button>
                </div>
