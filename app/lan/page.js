    ...
    <div className="min-h-screen bg-carbon overflow-hidden">
      ...
      <div className="relative w-full h-[90vh] flex flex-col md:flex-row items-center justify-between px-4 md:px-16 pt-20">
        ...
        <div className="relative z-10 max-w-2xl text-left md:w-1/2">
          <h1 className="text-5xl md:text-7xl font-syne font-extrabold text-cian uppercase tracking-tighter drop-shadow-2xl">ZONA LAN ÉLITE</h1>
          <p className="mt-6 text-xl font-inter text-white opacity-90 leading-relaxed">
            Experimenta el verdadero rendimiento. 50+ estaciones equipadas con NVIDIA RTX 4070, monitores Zowie 240Hz y sillas ergonómicas premium. Zero lag.
          </p>
        </div>

        {/* Right Widget */}
        <div className="relative z-10 w-full md:w-[400px] glass-panel p-8 rounded-sm shadow-2xl mt-8 md:mt-0">
          <h2 className="text-2xl font-orbitron text-cian mb-6 text-center uppercase tracking-widest">Reserva tu PC</h2>
          
          <form onSubmit={handleReservation} className="flex flex-col gap-4">
            <div>
              <label className="block text-cyber font-inter text-sm mb-1 uppercase tracking-wider">Fecha</label>
              <input 
                type="date" 
                value={date}
            ...
            <div className="flex gap-4">
              <div className="w-1/2">
                <label className="block text-cyber font-inter text-sm mb-1 uppercase tracking-wider">Hora</label>
                <select 
                  value={time}
                ...
                >
                  ...
                  <option value="22:00">10:00 PM (Amanecida)</option>
                </select>
              </div>
              <div className="w-1/2">
                <label className="block text-cyber font-inter text-sm mb-1 uppercase tracking-wider">Cant. PCs</label>
                <input 
                  type="number" 
                  min="1" 
                  max="10"
            ...
            <button type="submit" className="mt-4 w-full bg-cian text-carbon font-orbitron font-bold text-lg px-6 py-4 rounded-sm uppercase tracking-widest hover:shadow-glow-cian transition-all duration-300 transform hover:-translate-y-1">
              {success ? '¡Reserva Confirmada!' : 'Confirmar Reserva'}
            </button>
          </form>
        </div>
      </div>
      ...
      <div className="container mx-auto px-4 py-24 relative z-10">
        <h2 className="text-3xl font-syne text-white uppercase mb-12 border-l-4 border-cian pl-4">Próximos Torneos</h2>
        
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          {[
            { title: 'VALORANT MASTERS LIMA', date: 'Viernes 19, 18:00', prize: '$1,500 USD', img: 'https://images.unsplash.com/photo-1542751371-adc38448a05e?ixlib=rb-4.0.3' },
            { title: 'CS2 WEEKEND BRAWL', date: 'Sábado 20, 15:00', prize: '$500 USD', img: 'https://images.unsplash.com/photo-1511512578047-dfb367046420?ixlib=rb-4.0.3' },
          ].map((tourney, idx) => (
